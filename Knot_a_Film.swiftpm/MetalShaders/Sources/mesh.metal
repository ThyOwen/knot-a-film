...
[[mesh]] void meshLineShader( LineMeshType output,
    ...
    // 5 line primitives per thread
    output.set_primitive_count(count * 5);

    if (!payload.mask[tid])
        return;

    // 6 vertices per thread
    uint vertBase = tid * 6;

    uint v0 = vertBase + 0; // origin
    uint v1 = vertBase + 1; // midpoint
    uint v2 = vertBase + 2; // joint
    uint v3 = vertBase + 3; // prevJoint
    uint v4 = vertBase + 4; // prevMidpoint
    uint v5 = vertBase + 5; // origin (dup)

    VertexOut a, b, c, d, e, f;
    ...
    d.position = float4(payload.prevJoint[tid],    0.0, 1.0);
    e.position = float4(payload.prevMidpoint[tid], 0.0, 1.0);
    f.position = float4(payload.origin[tid],       0.0, 1.0);

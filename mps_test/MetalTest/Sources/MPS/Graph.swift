...
struct Graph {
    ...
    let graphParams : GraphParams<Float32>
    ...
    init(graphParams : GraphParams<Float32>) {
    ...
    mutating func testRun(positionsBuffer : inout MTLBuffer, velocitiesBuffer : inout MTLBuffer) {
    ...
    mutating func runInplace(positionsBuffer : inout MTLBuffer, velocitiesBuffer : inout MTLBuffer, on commandQueue : MTLCommandQueue, printBuffers : Bool = false) {
                
        guard let outputVelocities = self.outputVelocities, let outputPositions = self.outputPositions else {
            fatalError("tenors not initalized")
        }
    ...
    func compile(on metalDevice : MTLDevice, useFloat16 : Bool = false) {
        
        let type : MPSDataType = useFloat16 ? .float16 : .float32
        ...
        let executable = self.graph.compile(with: graphDevice,
                           feeds: inputFeeds,
                           targetTensors: [outputPositions, outputVelocities],
                           targetOperations: [],
                           compilationDescriptor: compilationDescriptor)
        
        let fileURL = FileManager.default.urls(for: .desktopDirectory, in: .userDomainMask).first!.appendingPathComponent("test.mpsgraphpackage")

        // Serialize the executable to the specified URL.
        executable.serialize(
            package: fileURL,
            descriptor: nil
        )

        print("MPSGraph executable saved to: \(fileURL.path)")
        
    }
    
    
    // builders
    mutating func buildGraphReducedPrecision() {
        let inputNodeShape: [NSNumber] = [self.numNodes, 2]
        
        let positionsTensor = self.graph.placeholder(shape: inputNodeShape, dataType: .float16, name: "positions") // [N, 2]
        let velocityTensor = self.graph.placeholder(shape: inputNodeShape, dataType: .float16, name: "velocities") // [N, 2]
        ...
        let deltas = self.graph.subtraction(bNodePositionsTiled, aNodePositionsTiled, name: "positions_delta") // [N, N, 2]
        ...
        let epsilon = self.graph.constant(self.graphParams.epsilon, dataType: .float16) //[N]
        let softenedDistanceSquared = self.graph.addition(distanceSquared, epsilon, name: "softer_distance_squared") // [N, N, 1]
        let distances = self.graph.squareRoot(with: softenedDistanceSquared, name: "distances") // [N, N, 2]
        
        let distancesTiled = self.graph.tileTensor(distances, withMultiplier: [1, 1, 2], name: "distances_tiled") // [N, N, 2]
        
        let distancesSqueezed = self.graph.squeeze(softenedDistanceSquared, axis: 2, name: "distances_squeezed") // [N, N]

        let directions = self.graph.division(deltas, distancesTiled, name: "directions")
        
        //calculate forces between every node and every other node
        let edgeRepulsionTensor = self.graph.constant(self.graphParams.edgeRepulsion, dataType: .float16)
        let repulsionForce = self.graph.division(edgeRepulsionTensor, distancesSqueezed, name: "repulsion_force") // [N, N, 2]

        let forcesExpanded = self.graph.expandDims(repulsionForce, axis: 2, name: "force_expanded") // [N, N, 1]
        let acceleration = self.graph.multiplication(forcesExpanded, directions, name: "acceleration") // [N, N, 2] change in velocity F/m; m = 1 so just a
        
        let perNodeAcceleration = self.graph.reductionSum(with: acceleration, axis: 0, name: "per_node_acceleration") // [N, 1, 2]

        let perNodeAccelerationSqueezed = self.graph.squeeze(perNodeAcceleration, axis: 0, name: "per_node_acceleration_squeezed") // [N, 2]
        
        let updatedVelocities = self.graph.addition(velocityTensor, perNodeAccelerationSqueezed, name: "updated_velocities") // [N, 2]
        
        let damping = self.graph.constant(self.graphParams.damping, dataType: .float16) // [1]
    ...
    mutating func buildGraphStandard(sparse : Bool = true) {
        let inputNodeShape: [NSNumber] = [self.numNodes, 2]
        
        let positionsTensor = self.graph.placeholder(shape: inputNodeShape, dataType: .float32, name: "positions") // [N, 2]
        let velocityTensor = self.graph.placeholder(shape: inputNodeShape, dataType: .float32, name: "velocities") // [N, 2]
        ...
        self.connections = edgeIndices
        let numEdges = NSNumber(value: edgeIndices.count)
        
        let edgeAttractionTensor = self.withTupledAccess(to: edgeIndices) { columnTensor, rowTensor in
    ...
    mutating func buildGraphNew() {
        ...
        let acceleration = self.graph.multiplication(repulsionForce, directions, name: "acceleration") // [N, N, H, W, 1]
    ...
    static func getTileBounds<N : FloatingPoint & Numeric>(for params : GraphParams<N>.TileParams) -> [(N, N)] {
    ...
    internal mutating func printBuffer(outputTensor : MPSGraphTensor, outputTensorData : MPSGraphTensorData) {
